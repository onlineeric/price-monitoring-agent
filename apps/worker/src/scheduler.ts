...
import { cronPatternToDescription, type EmailScheduleSettings, settingsToCronPattern } from "./utils/cronConverter.js";
...
export class DigestScheduler {
  ...
  // biome-ignore lint/correctness/noUnusedPrivateClassMembers: kept for debug/observability of the active repeat key
  ...
  private async fetchScheduleSettings(): Promise<EmailScheduleSettings | null> {
    try {
      ...
      const scheduleSetting = await db.select().from(settings).where(eq(settings.key, "email_schedule")).limit(1);
