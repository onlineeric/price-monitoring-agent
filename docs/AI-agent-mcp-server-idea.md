...
- [x] 3.14 **[Manual]** Local smoke test. (a) `pnpm docker:up && pnpm mcp:up`. (b) `curl http://localhost:3001/health` → expect `200` with `{ status: "ok", ... }`. (c) Set `MCP_HTTP_URL=http://localhost:3001/mcp` in `apps/web/.env.local`, restart web dev, send a chat message that calls a tool. (d) Regression check: unset `MCP_HTTP_URL`, verify VSCode/Cursor stdio integration still works after the 3.9 refactor.
