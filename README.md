...
- Retrieval-Augmented Generation (RAG): semantic product search over pgvector, with a locally-hosted embedding model so the chat agent finds products by *meaning*, not just keywords
- Rich structured metadata extraction (description, category, brand, country of origin, key/value specs) decoupled from cheap, frequent price checks
- Conversational AI agent with tool calling over a custom Model Context Protocol (MCP) server, streaming UI, and clickable product cards wired back into the app
- Background job orchestration for price checks, metadata enrichment, vector reindexing, digest generation, and scheduler-managed repeatable jobs
- Browser automation with Playwright Extra and stealth mode for difficult e-commerce pages
- Production-oriented operations including Dockerized services, health endpoints, versioned auto-applied migrations, CI/CD, and self-hosted deployment
